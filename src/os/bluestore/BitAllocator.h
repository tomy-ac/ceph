...
class BitMapList {
  std::vector<BitMapEntity> *m_items;
public:
  BitMapList(std::vector<BitMapEntity> *list, int64_t len, int64_t marker) {
    m_items = list;
  }

  ~BitMapList() { }

  BitMapEntity *get_nth_item(int64_t idx) {
    return &(*m_items)[idx];
  }
  ...
  std::vector<BitMapEntity> *get_list() {
    return m_items;
  }
};
...
class BitMapEntityIter {
  BitMapList <BitMapEntity> *m_list;
  int64_t m_start_idx;
  int64_t m_cur_idx;
  bool m_wrap;
  bool m_wrapped;
...
public:
  ...
  void init(BitMapList<BitMapEntity> *list, bool wrap, int64_t start_idx) {
    m_list = list;
    m_wrap = wrap;
  ...
  BitMapEntityIter(BitMapList<BitMapEntity> *list, int64_t start_idx) {
    init(list, false, start_idx);
  }
  BitMapEntityIter(BitMapList<BitMapEntity> *list, int64_t start_idx, bool wrap) {
...
private:
  ...
  BitMapList<BmapEntry> *m_bmap_list;
  std::atomic_char m_lock;
